...
public class AuthFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (uri.getPath().startsWith("/auth") || uri.getPath().startsWith("/auth/login")) {
        ...
        JwtTokenVO authorization = authClientApi.authorization();
        // 权限验证逻辑，忽略，这里直接返回验证不通过
//        if (!responseWrapped.isSuccess() && !Objects.isNull(responseWrapped.getData())) {
//            return this.doLoginVoidMono(exchange);
//        }
        // 认证通过，设jwt身份信息令牌
        request.getHeaders().add(JwtTokenVO.AUTHORITY_TOKEN, authorization.getAuthorityToken());
        return chain.filter(exchange);
    }
