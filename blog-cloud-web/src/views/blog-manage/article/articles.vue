...
import { requestGet ,currentPOSTPath} from "@/api"
...
const getBlog = () => {
    ...
    requestGet("iBlogList", param).then((res:any) => {
        // console.log(res);
        if (res.code == 200) {
        ...
        <el-tabs v-model="blogData.publish" class="articles-tabs" @tab-click="tabClick">
            <el-tab-pane label="文章" name="1">
                <ul class="articles_list">
                    <li v-for="item in blogData.list" :key="item.id" class="articles_list_item">
                        <h2 class="title">
                            <!-- @click="jump('/article/details', item)" -->
                            <router-link tag="详情" target="_blank" :to="'/article/details/' + item.id">
                        ...
                        <div class="flex justify-between">
                            ...
                            <div class="actions flex">
                                <div class="mr-2 item" @click="jumpEdit('/write/essay', item)">编辑</div>
                                <div class="mr-2 item" @click="jump('/article/details', item)"> 浏览</div>
                                <i class="not-italic item">
                                    <el-dropdown @command="(commands: string | undefined) => command(commands, item)">
                ...
                <div class="flex items-center justify-center mt-4">
                    <el-pagination background layout="prev, pager, next" :page-size="blogData.size"
                        :total="blogData.total" @current-change="currentChange" hide-on-single-page />
            ...
            <el-tab-pane label="草稿箱" name="0">
                <ul class="articles_list">
                    <li v-for="item in blogData.list" :key="item.id" class="articles_list_item">
                        <h2 class="title">
                            <router-link tag="详情" target="_blank" :to="'/write/essay?articleId=' + item.id">
                                {{ item.title }}</router-link>
                        </h2>
                        <div class="flex justify-between">
                            <div class="info-box_action flex items-center">
